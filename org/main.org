* TODO data bug: those "repayment" variables are not
They are when the loan started, not when it ended.
...
***** DONE exempt v. deduction: solved
   ...
   deduccion : se puede restar del base gravable. gastos so deudcible (o no) de los ingresos.
...
*** TODO dividend income
dividend income was grouped into capital income; it's not that, it has a separate schedule (Tarifa 3). It carries no deductions, no exemptions.
** the deduction applies only to labor or contract income, but the tax is paid on more kinds
** in David's Excel model, trace the formulas for "inputs to the marginal tax schedule"
** assign dependents to income earners
*** verify with Juan that non-govt income determines dependence
...
** there are two kinds of deduction. maybe they are these:
1 - subtract from the taxable base
2 - subtract from the tax liability
** three|five kinds of income
each with a different base (deductions vary across them) and a different schedule
*** labor income
**** major deductions
everything paid as an employee contribution to social security: deduct from base
  includes health, pension, solidarity
25% of labor income. PITFALL: Order matters.
*** pension income
same schedule as labor, different exemptions
*** capital income
e.g. rental income
*** dividend income: 2 subtypes
**** "fiscal profit" of the firm - taxes paid by the firm
"fiscal profit" is not the normal accounting profit; it's some legal figment.
