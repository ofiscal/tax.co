* TODO eliminate manual boolean flags determining behavior
add strategy suffix to all files downstream of vat_rates
  purchases_2_vat
  purchase_sums
  purchase_pics
  people_3_purchases
...
output structure
* TODO "add to all the results Median (VAT/Income) with median VAT, and Average (VAT/Income) with average VAT"
* TODO ? is vat too low
** DONE what if we exclude capitulo c files
  no, those are a very small fraction of total purchases
>>> subsample = 10
...
** what if we delete those 98 and 99 values
If they are in the "value" column, they should not make a difference for overall purchase volume.
But maybe they're in some other column?
* summary table
** variables:
  income
  Labor + Cesantia
  capital w/o dividends
  Dividend
  Pension
  govt
  private
...
*** place:
    ...
    Ibagué
** strategy
let vars = list of names of variables of interest
let groupList = []
for each kind of group (e.g. decile)
  let varList = []
  for each var in vars:
    let df = run tabulate_stats_by_group( households, kind-of-group, var )
    prefix the word var to every column name in df
    insert df into varList
  insert into groupList the horizontal (widening) concatenation of varList.values()
return the vertical (lengthening) concatenation of groupList
transpose
  now groups are columns, quantities summarized are rows
keep only the rows and columns of interest
  ditch the group "non-female head of household"
  ditch lots of rows
* TODO next
** add gpl license to code
...
** change output/vat-tables to output/vat/tables
./vat/report/pics/people.py:                 "output/vat-tables/recip-" + str(subsample),
./vat/report/pics/households.py:    "output/vat-tables/recip-" + str(subsample),
./vat/report/pics/households.py:    "output/vat-tables/recip-" + str(subsample),
./vat/report/pics/households.py:    "output/vat-tables/recip-" + str(subsample)
...
** PITFALL ! people["non-beca sources"] sometimes turns numeric
...
** update coicop-vat bridge on OneDrive
