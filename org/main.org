...
*** TODO remove people_5*.py
*** TODO rename people_4_ss -> people_4_income_taxes
...
*** TODO count the number of dependents in a household. merge that into person data.
Start a new file, people_5_dependents.py.
Read in the output from people_4_ss.
In a copy of it, keep only the household id and the dependent column.
Group that file by household, summing the number of dependents.
Merge that on household id back into the person data.
*** TODO for each of the top five earners, compare their income rank to the number of dependents
If their rank (1 means highest earner) is less than or equal to the number of dependents, they can claim a dependent.
...
* TODO descriptive stats on pensions
...
** to identify dependents, we assume that by "income < 260 UVT" the tax code means ...
that someone can be claimed as a dependent if their income from anything but government transfers is below 260 UVT. To whit, in build.people.main:
  ppl["dependent"] = (
