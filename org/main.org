* TODO ? maybe bug
In households.py, why are the income-rank variables summed across household-members?
(I think it's that the rank-n value will be 0 for every member except the one with rank n,
so the sum is equivalent to running "first" after sorting on that variable, and saves the work of sorting.)
* code review
** have read through
common.py
classes.py
