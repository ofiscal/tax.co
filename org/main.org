...
for each kind of group (e.g. decile)
  ...
  for each var in vars:
    ...
    insert into varDict the association ( var : df )
  insert into groupDict the horizontal concatenation of varDict.values()
    that is, to end up with lots of columns, all with the same groups
return the vertical concatenation of grupdDict.values()
transpose
