* TODO ? (juan investigating): The "repayment" income might not be that
They might be when the loan started, not when it ended.
** TODO PITFALL: Double-counting, no matter which interpretation we use.
* TODO personal income tax
** TODO ? are there SS deductions to apply from income-tax.org?
** verify: no deductions, exemptions for ss contribs?
** how should we use P6871? (Juan is investigating)
** how to tax? (Juan is investigating)
