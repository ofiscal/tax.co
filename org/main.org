...
** TODO (juan investigating): Borrowed "income"
*** TODO It is currently called "repayment", but it's the opposite: borrowing.
*** TODO Is it taxed?
*** TODO PITFALL: Double-counting.
** TODO (show juan) Are there more applicable SS deductions in income-tax.org?
** impuesto de ganancia ocasional
*** 10% flat rate, no deductions, no exemptions.
*** variables
P7513S9A1 (gambling)
P7513S10A1 (inheritance)
** impuesto de indemnizacion
P7513S8A1 (jury awards)
** impuesto sobre donaciones
tax = (S - min( S / 5, 2290 uvt)) *
  where S = sum of all gifts (private or public)
          = P7510S3A1 + P7510S4A1
** TODO (juan investigating)
  , "P7510S1A1"  : "income, year : private : remittance, domestic" – Uncertain; Juan will ask lawyer. se debe contabilizar como ingreso (otros ingresos), pagan impuesto tarifa 2 si superan 1.400 UVT, se considera ingreso no laboral.
  , "P7510S2A1"  : "income, year : private : remittance, foreign" - Uncertain; Juan will ask lawyer. se debe contabilizar como ingreso (otros ingresos), pagan impuesto tarifa 2 si superan 1.400 UVT, se considera ingreso no laboral. (discontinuity?)
** ignorable
*** special
...
** the two stock variables
*** they should be redundant.
The two questions record the same information; one of them should always be zero, or if they’re both nonzero, they should be the same number.
*** first check them
Check that no row contains two unequal nonzero values.
*** to use them after checking
Compute the maximum and just use that instead of the two of them.
** TODO pension + labor
*** TODO labor
...
**** cesantias: exempt when firm sends to the "fondo de cesantias", but not when withdrawn
and what we have in the ENPH is withdrawals
**** GMF = 4 por mil. Deduct half.
**** deduct from labor income
 everything paid as an employee contribution to social security: deduct from base
...
**** absent from ENPH
 ...
 aportes voluntarios a fndos de cesantias (deduccion hasta 1/12 del ingreso)
*** pension
If response to P6110 is 2, then deduct value in P6120 from pension income before computing taxes.
*** apply Tarifa 1 to (labor + pension), not to each individually
** nonlabor income
*** general procedure
Tarifa 2, just like capital income.
Deduct appropriate things from capital income, and then add nonlabor income (for which the law makes room for subtracting deductions, but for which we know of no actual deductions) before applying Tarifa 2.
...
****** all the "capital income" in the code is in fact capital income, except maybe
the variables currently called "repayment"
...
So far we've been grouping all capital income together, but it has to be split, because sales and honorarios cannot have the GMF deducted, while the "normal" capital income can.
...
*** any kind of income -- govt transfers, becas, in-kind -- determines dependence
*** TODO ? include non-child family with income > 260 UVT if they say they're unemployed b/c illness?
Otherwise we're treating child family differently from non-child family.
But if we do this, there are disabled people with total income (not labor income) > 260 UVT / year.
The kind of income that gets taxed, and the kind that can disqulify someone from being a dependent, matter.
...
*** ignore P6871
It describes the frequency with which monthly income is disbursed; it does not bear on the monthly total.
* DONE (right?): descriptive stats on pensions
These are in 'caracteristicas generales'.
