...
*** DONE Don't integrate it into the Makefile
The logic would be complex and we won't be using it for a long time.
*** Instead, use a separate Makefile.goods-by-decile and a separate make-goods-by-decile.sh
**** Makefile.goods-by-decile
It duplicates relevant parts of the Makefile.
