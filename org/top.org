...
*** max 22 in raw people
>>> pp0 = oio.readStage( 1, 'people_0')
>>> pp0["household-member"].max()
22
*** max 120 in raw purchases
>>> ps0 = cl.collect_files(
...   ( articulos.files
...     + capitulo_c.files
...     + nice_purchases.files )
...   , subsample = 1
... )
>>> ps0["household-member"].max()
120
*** max 195 in purchases_1
>>> ps1 = oio.readStage( 1, 'purchases_1')
>>> ps1["household-member"].max()
195
