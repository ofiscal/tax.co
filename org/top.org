...
** TODO new Pandas
it's already built
*** Correction.Replace_Missing_Values: converts to float
Upgrade pandas so that np.nan can be int also.
Once this is solved, update classes_test.py too.
** coverage
*** complete
build/buildings.py
** TODO : Replace_Missing_Values is string-unaware
*** example, from build.purchases.main
  [ Correction.Replace_Substring_In_Column(
      "quantity", ",", "." )
  # INSERT HERE a command to change quantity from string to number
  # (ideally int, but until I have the new version of Pandas built,
  # it will have to be float.)
...
*** TODO verify that coicop and 25-broad-categs are never both missing
** pitfalls
