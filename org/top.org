* TODO test suite
** TODO mystery: Why is estrato sometimes an int?
*** that problem
If subsample=1000, it's int64.
Otherwise it's float64.
See build.buildings_test.
*** TODO solution ? never use a subsample for the buildings output file
Currently, the full sample is always used as the input to build/buildings.py.
Maybe the same should be true of its output.
**** TODO If that's true, must change all these files
buildings.py (output)
buildings_test.py (input)
Makefile
maybe whatever is downstream of buildings.py -- probably households.py
