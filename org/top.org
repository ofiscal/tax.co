* TODO test suite
** TODO Rewrite so that all IO comes at the end, for Makefile soundness.
*** motivation:: build/purchases_2_vat_test.py does not re-run after failing
**** to see this
  set some assertion to always fail
  e.g. set the number on line 83 to 0
*** how to change it: Mirror the changes to build/purchases_2_vat_test.py
Explicitly:
  During each test file, build a log as a string.
  Write it to disk only once all that file's tests have succeeded.
*** TODO non-test programs might have a similar problem
*** TODO once that's done, delete oiio.test_clear()
