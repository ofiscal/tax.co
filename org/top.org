* TODO use make.py
** build latest Dockerfile, with python-from-here
It's already written, but it won't build from the Nix system.
(It starts from scratch rather than using cached layers,
and runs into an apt problem.)
