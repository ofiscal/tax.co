* TODO test csvtool
See if what it generates from data/enph-2017/2_unzipped/ssv/
is what I've already got at data/enph-2017/3_csv.
* TODO update the javeriana download of the ENPH
Currently it contains some slightly processed data.
That's confusing and unnecessary, given the Makefile.
Change it to the raw data instead.
* TODO refactor .gitignore file(s)
Ignore most .xls etc. files, but provide some exceptions.
https://stackoverflow.com/questions/2415873/exceptions-in-gitignore
* TODO verify Makefile runs each test file
* TODO docker
** TODO download new docker image to desktop
** TODO update tax.co docker image
 Github's dependencybot says Pillow 6.1.0 has security vulnerabilities.
* TODO retire branch that was for reporter
** TODO document branch: average-vat-spending-per-rate
** goal
Average (across households) fraction of spending at each tax rate.
...
* TODO possibly wrong
** TODO vat_rates: maybe bad indentation
Search the TODO items for this:
...
** TODO assumption: set "where got" to "purchase"
...
*** TODO are therefobsolete (sep = ";") instructions now?
For example, in ./build/vat_rates.py, there's:
  vat_coicop = pd.read_csv( "data/vat/" + "vat-by-coicop.csv"
                          , sep = ";" # TODO PITFALL
                          , encoding = "latin1" )
But that's not reading the ENPH; it's reading the VAT-COICOP bridge.
Is there anything that reads the ENPH using sep=;?
*** TODO document
**** csvtool deletes whitespace between separators
...
* TODO refactor
** TODO do we use the SS schedule?
