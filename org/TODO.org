...
** TODO IsNull should be excised, replaced with MissingAtMost
and InRange changed to accept null values.
But this requires adding MissingAtMost tests everywhere IsNull was used.
** TODO homogenize tests
For each program, use
  InRange, CoversRange, MeanBounds, MissingAtMost
for all vars,
and test number of obs and column names.
*** TODO ? household_1_agg_plus: include such tests for means of incomes
Seems kind of overkill, since they are already tested by ensuring that they sum to what they sum to in the people data.
** TODO add this test for income rank in people data
...
** TODO test means
Test that the mean of each variable is in some range.
** TODO maybe
*** TODO ? people.main:
