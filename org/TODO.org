* TODO merge master and time-to-save
** comparing branches
...
***** TODO test that variable
**** python/build/people_2_buildings_test.py
**** python/build/purchase_sums_test.py
*** TODO it's probably better in
**** python/build/buildings.py
 It pulls in one more variable,
 + , ("P5102", 0, "recently bought this house", 0) ].
***** TODO : testt that variable
*** TODO it's not exactly better in
**** python/build/households_1_agg_plus.py
 The only difference is that it uses two more variables,
 but it does so awkwardly:
   + + [ "used savings"
...
**** Unlike master, time-to-save does the following
***** it defines this variable, which never gets used
  purchases["home purchase value"] = ...
****** TODO ? test it, or leave a note that it's untested
***** it retains "tax-purchases", if indeed there are any
...
In this sense every variable that is corriente should be the usual income or expending, the definition is "es igual al gasto total del hogar, excepto pagos de contado por ampliación o subdivisión de vivienda, pagos de contado o cuota inicial por compra de vivienda y compra de automóvil o motocicleta para uso particular." so everything except unusuall expenditure in huge things
the Unidad de Gasto is the household, It should be the same for each Directorio
Disponible is after law deductions, wich might be pretty usefull
If those things are correct, and I am almost sure they are the definition of each variable sholud be the following:
Ingreso Corriente de la Unidad de Gasto: All the income Monetarie or not (gifts, and material support) per household
Ingreso Corriente Monetario de la Unidad de Gasto: same as before but just Monetarie
Ingreso Corriente Monetario Disponible de la Unidada de Gasto: same as before but after taxes and deductions
Gasto Total de la Unidada de Gasto: Total expendature for the household
Gasto Corriente de la Unidad de Gasto: expenses without huge boughts like houses or vehicles per household
Gasto Corriente Monetario de la Unidad de Gasto: Same as before but just the things that were bought with money directly
