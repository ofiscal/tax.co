* use Vaex to reduce memory load
Vaex uses disk more, and RAM less.
...
** TODO magic numbers
I found them all. Only one is in production. What remains is just to document them.
*** how to find them
In tmux, run
...
*** them
The categorical variables like where-got.
  These are magic in the ENPH data. They should remain.
build/purchases/main_defs.py: absurdly_big_expenditure_threshold = 1e9
build/accumulate_marginal.py
  This is unused.
** TODO move unused files to an exemplars/ folder
accumulate_marginal.py
house stuff
