...
* TODO possibly wrong
** TODO use "has dependent"
Dependents: 10% of “renta gravable laboral” until 32 UVT
*** TODO diff people_4_income_taxish.detail.2018.csv appropriately
This requires creating a composite key from household and household-member,
so that I can pass that as the key to csv-diff.
**** code to create diff from bash
csv-diff   data/recip-10/people_4_income_taxish.detail.2018.csv \
           no-dep-exempt/people_4_income_taxish.detail.2018.csv \
    --key="household"
**** BUGGY code to create diff within python
I suspect it doesn't want multiple keys.
...
diff = compare(
  ...
  load_csv("no-dep-exempt/people_4_income_taxish.detail.2018.csv",
           key=["household","household-member"] ) )
*** TODO ongoing: implementing for 2018, not 2016
I need someone to figure out the rules for 2016,
since for that year there are many kinds of labor income;
surely the same exemption shouldn't apply to each one.
