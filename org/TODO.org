...
* TODO test suite
** TODO RESUME here
*** test number of purchases
Divide purchases/main into files `collect` and `correct`.
For `collect`, test only the number of rows.
For `correct`, migrate the other tests, and add one of the number of rows.
Add the same test to later purchase-level files.
