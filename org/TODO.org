* TODO merge purchases into households, not people
** TODO purchase_sums_test not rerunning upon edit
** current situation
*** build/purchase_sums
**** currently injects these variables into the people data
 value
 transactions
 vat paid, max
 vat paid, min
 predial
*** build/people_3_purchases
**** modifies "vat paid" for San Andres
**** creates more vat variables
...
*** build/people_4_income_taxish doesn't use any of it
*** build/households
**** groups some of those variables
**** creates new variables that use the purchase info
...
** TODO how to change
*** DONE purchase_sums: Sum to household level.
*** DONE people_3_purchases: ditch the merge
And save in moving.py the code that therefore can't be used here.
*** TODO households: make a second stage
Recreate everything in moving.py.
Result should plug into overview.py without generating complaints.
*** TODO to test that the aggregated purchases lose nothing
Compare the sum of a column in the original with same in the aggregated.
** TODO after that, delete people_3_purchases
Move its code to an adjacent program.
...
* TODO test suite
** TODO RESUME here
*** BUG : exposed by python/build/purchase_sums_test.py
suspicion: it's because there are more household members in the purchase data than in the person data
*** test number of households
*** some tests fail on the 1/1000 sample
